...
Blockly.telesphorus.RenderInfo.prototype.addElemSpacing_ = function () {
  ...
  for (let i = 0, row;
    ...
    for (let e = 0; e < oldElems.length - 1; e++) {
      ...
      let spacing = this.getInRowSpacing_(oldElems[e], oldElems[e + 1])
      row.elements.push(
...
Blockly.telesphorus.RenderInfo.prototype.getElemCenterline_ = function (row, elem) {
  ...
  if (Blockly.blockRendering.Types.isBottomRow(row)) {
    let baseline = row.yPos + row.height - row.descenderHeight
  ...
  if (Blockly.blockRendering.Types.isField(elem) && row.hasStatement) {
    let offset = this.constants_.TALL_INPUT_FIELD_OFFSET_Y +
      elem.height / 2
    result += offset
  } else {
    result += (row.height / 2)
...
Blockly.telesphorus.RenderInfo.prototype.finalize_ = function () {
  ...
  for (let i = 0, row;
    ...
    let heightWithoutHat = yCursor - this.topRow.ascenderHeight
    if (row === this.bottomRow &&
      heightWithoutHat < this.constants_.MIN_BLOCK_HEIGHT) {
      // But the hat height shouldn't be part of this.
      let diff = this.constants_.MIN_BLOCK_HEIGHT - heightWithoutHat
